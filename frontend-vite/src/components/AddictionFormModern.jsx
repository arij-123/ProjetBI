import React, { useState, useEffect, useCallback } from 'react';
import { Button } from './ui/Button';
import { Card, CardHeader, CardBody, CardFooter } from './ui/Card';
import { RangeSlider } from './ui/RangeSlider';
import { cn, downloadFile, copyToClipboard } from '../lib/utils';

const AddictionFormModern = () => {
  const [formData, setFormData] = useState({
    age: 25,
    work_study_hours: 8,
    daily_screen_time: 4,
    app_sessions: 50,
    social_media_usage: 2,
    gaming_time: 1,
    notifications: 100,
    night_usage: 1,
    stress_level: 5,
    apps_installed: 30
  });

  const [results, setResults] = useState({
    score: 0,
    riskLevel: '',
    recommendations: [],
    categoryScores: {},
    prediction: '',
    confidence: '',
    probability: 0,
    timestamp: '',
    processingTime: 0
  });

  const [isCalculating, setIsCalculating] = useState(false);
  const [apiError, setApiError] = useState(null);
  const [connectionStatus, setConnectionStatus] = useState('disconnected');
  const [retryCount, setRetryCount] = useState(0);
  const [showResults, setShowResults] = useState(false);

  const handleInputChange = useCallback((field, value) => {
    const numValue = field === 'age' || field === 'app_sessions' || field === 'notifications' || field === 'stress_level' || field === 'apps_installed' 
      ? parseInt(value) 
      : parseFloat(value);
    
    setFormData(prev => ({
      ...prev,
      [field]: numValue
    }));
  }, []);

  const checkConnection = useCallback(async () => {
    try {
      const response = await fetch('http://localhost:8000/health');
      if (response.ok) {
        setConnectionStatus('connected');
        setApiError(null);
        return true;
      }
    } catch (error) {
      setConnectionStatus('disconnected');
      return false;
    }
  }, []);

  const callPredictionAPI = useCallback(async () => {
    setIsCalculating(true);
    setApiError(null);
    setShowResults(false);
    
    const isConnected = await checkConnection();
    if (!isConnected) {
      setApiError('Impossible de se connecter au serveur ML. Vérifiez que le backend est démarré sur http://localhost:8000');
      return;
    }
    
    try {
      const response = await fetch('http://localhost:8000/predict', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(formData),
      });
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || `Erreur HTTP: ${response.status}`);
      }
      
      const data = await response.json();
      
      setResults({
        score: Math.round(data.probability * 100),
        riskLevel: data.risk_level,
        recommendations: data.recommendations || [],
        categoryScores: data.feature_importance || {},
        prediction: data.prediction,
        confidence: data.confidence,
        probability: data.probability,
        timestamp: data.timestamp,
        processingTime: data.processing_time_ms
      });
      
      setRetryCount(0);
      setShowResults(true);
      
    } catch (error) {
      console.error('Erreur API:', error);
      setApiError(error.message || 'Erreur de connexion au serveur ML.');
      
      if (retryCount < 2) {
        setRetryCount(prev => prev + 1);
        setTimeout(() => callPredictionAPI(), 2000 * (retryCount + 1));
      }
    } finally {
      setIsCalculating(false);
    }
  }, [formData, checkConnection, retryCount]);

  const calculateAddictionScore = () => {
    callPredictionAPI();
  };

  const saveToLocalStorage = () => {
    try {
      localStorage.setItem('addictionFormData', JSON.stringify(formData));
      localStorage.setItem('addictionResults', JSON.stringify(results));
      showNotification('Profil sauvegardé localement !', 'success');
    } catch (error) {
      console.error('Erreur lors de la sauvegarde:', error);
      showNotification('Erreur lors de la sauvegarde locale', 'error');
    }
  };

  const downloadReport = () => {
    const report = {
      date: new Date().toLocaleDateString('fr-FR'),
      formData,
      results,
      connectionStatus,
      generatedAt: new Date().toISOString()
    };
    
    downloadFile(
      JSON.stringify(report, null, 2), 
      `rapport-addiction-${new Date().toISOString().split('T')[0]}.json`
    );
    showNotification('Rapport téléchargé !', 'success');
  };

  const shareResults = async () => {
    const text = `Mon score d'addiction mobile: ${results.score}/100 (${results.riskLevel})`;
    
    try {
      if (navigator.share) {
        await navigator.share({ text });
      } else {
        await copyToClipboard(text);
        showNotification('Résultats copiés dans le presse-papiers !', 'success');
      }
    } catch (error) {
      showNotification('Erreur lors du partage', 'error');
    }
  };

  const resetForm = () => {
    setFormData({
      age: 25,
      work_study_hours: 8,
      daily_screen_time: 4,
      app_sessions: 50,
      social_media_usage: 2,
      gaming_time: 1,
      notifications: 100,
      night_usage: 1,
      stress_level: 5,
      apps_installed: 30
    });
    setResults({
      score: 0,
      riskLevel: '',
      recommendations: [],
      categoryScores: {},
      prediction: '',
      confidence: '',
      probability: 0,
      timestamp: '',
      processingTime: 0
    });
    setShowResults(false);
    setApiError(null);
  };

  const showNotification = (message, type = 'info') => {
    const notification = document.createElement('div');
    notification.className = cn(
      'alert fixed top-4 right-4 z-50 min-w-[300px] animate-slide-down',
      type === 'success' && 'alert-success',
      type === 'error' && 'alert-error',
      type === 'info' && 'alert-info'
    );
    notification.innerHTML = `
      <div class="flex items-start gap-3">
        <i class="fas ${
          type === 'success' ? 'fa-check-circle text-success-600' : 
          type === 'error' ? 'fa-exclamation-circle text-accent-600' : 
          'fa-info-circle text-primary-600'
        } mt-0.5"></i>
        <div class="flex-1">
          <p class="font-medium">${message}</p>
        </div>
        <button class="text-secondary-400 hover:text-secondary-600" onclick="this.parentElement.parentElement.remove()">
          <i class="fas fa-times"></i>
        </button>
      </div>
    `;
    
    document.body.appendChild(notification);
    
    setTimeout(() => {
      if (notification.parentElement) {
        notification.classList.add('animate-fade-out');
        setTimeout(() => notification.remove(), 300);
      }
    }, 5000);
  };

  useEffect(() => {
    checkConnection();
    const interval = setInterval(checkConnection, 30000);
    return () => clearInterval(interval);
  }, [checkConnection]);
  
  useEffect(() => {
    const savedData = localStorage.getItem('addictionFormData');
    const savedResults = localStorage.getItem('addictionResults');
    
    if (savedData) {
      try {
        setFormData(JSON.parse(savedData));
      } catch (error) {
        console.error('Erreur lors du chargement des données sauvegardées:', error);
      }
    }
    if (savedResults) {
      try {
        setResults(JSON.parse(savedResults));
        setShowResults(true);
      } catch (error) {
        console.error('Erreur lors du chargement des résultats sauvegardés:', error);
      }
    }
  }, []);

  const formSections = [
    {
      title: 'Informations Personnelles',
      icon: 'fa-user',
      fields: [
        { key: 'age', label: 'Âge', min: 13, max: 80, unit: ' ans', color: 'primary' },
        { key: 'work_study_hours', label: 'Heures travail/étude', min: 0, max: 16, step: 0.5, unit: 'h', color: 'secondary' }
      ]
    },
    {
      title: 'Utilisation Mobile',
      icon: 'fa-mobile-alt',
      fields: [
        { key: 'daily_screen_time', label: 'Temps d\'écran quotidien', min: 0, max: 12, step: 0.5, unit: 'h', color: 'accent' },
        { key: 'app_sessions', label: 'Sessions par jour', min: 0, max: 200, step: 5, unit: '', color: 'primary' },
        { key: 'social_media_usage', label: 'Réseaux sociaux', min: 0, max: 8, step: 0.5, unit: 'h', color: 'secondary' },
        { key: 'gaming_time', label: 'Gaming', min: 0, max: 8, step: 0.5, unit: 'h', color: 'success' }
      ]
    },
    {
      title: 'Comportement',
      icon: 'fa-brain',
      fields: [
        { key: 'notifications', label: 'Notifications/jour', min: 0, max: 500, step: 10, unit: '', color: 'warning' },
        { key: 'night_usage', label: 'Usage nocturne', min: 0, max: 6, step: 0.5, unit: 'h', color: 'primary' },
        { key: 'stress_level', label: 'Niveau de stress', min: 1, max: 10, unit: '/10', color: 'accent' },
        { key: 'apps_installed', label: 'Apps installées', min: 0, max: 200, step: 5, unit: '', color: 'secondary' }
      ]
    }
  ];

  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-secondary-50">
      {/* Header */}
      <header className="bg-white shadow-soft sticky top-0 z-40 safe-top">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-gradient-primary rounded-xl flex items-center justify-center">
                <i className="fas fa-mobile-alt text-white"></i>
              </div>
              <div>
                <h1 className="text-xl font-bold text-gradient">Addiction Mobile</h1>
                <p className="text-sm text-secondary-600">Analyse ML intelligente</p>
              </div>
            </div>
            <div className={cn(
              'connection-status',
              connectionStatus === 'connected' ? 'connected' : 'disconnected'
            )}>
              <i className={cn(
                'fas text-sm',
                connectionStatus === 'connected' ? 'fa-check-circle' : 'fa-exclamation-triangle'
              )}></i>
              <span className="text-xs font-medium">
                {connectionStatus === 'connected' ? 'Connecté' : 'Hors ligne'}
              </span>
            </div>
          </div>
        </div>
      </header>

      {/* Main Content */}
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        {/* Hero Section */}
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-secondary-900 mb-4">
            Évaluez votre niveau d'addiction mobile
          </h2>
          <p className="text-xl text-secondary-600 max-w-3xl mx-auto">
            Notre système de machine learning analyse vos habitudes d'utilisation pour vous fournir 
            une évaluation précise et personnalisée.
          </p>
        </div>

        {/* Form */}
        <Card className="mb-8" hover>
          <CardHeader className="text-center pb-6">
            <h3 className="text-2xl font-bold text-secondary-900 mb-2">
              <i className="fas fa-chart-line mr-2 text-primary-600"></i>
              Questionnaire d'Évaluation
            </h3>
            <p className="text-secondary-600">
              Ajustez les curseurs pour refléter votre utilisation quotidienne
            </p>
          </CardHeader>
          
          <CardBody className="space-y-8">
            {apiError && (
              <div className="alert alert-error">
                <i className="fas fa-exclamation-triangle"></i>
                <div>
                  <p className="font-medium">Erreur de connexion</p>
                  <p className="text-sm">{apiError}</p>
                </div>
                <button 
                  onClick={() => setApiError(null)}
                  className="text-secondary-400 hover:text-secondary-600"
                >
                  <i className="fas fa-times"></i>
                </button>
              </div>
            )}

            <form onSubmit={(e) => { e.preventDefault(); calculateAddictionScore(); }}>
              {formSections.map((section, sectionIndex) => (
                <div key={sectionIndex} className="bg-secondary-50 rounded-2xl p-6">
                  <h4 className="text-lg font-semibold text-secondary-900 mb-6 flex items-center">
                    <i className={cn('fas mr-2', section.icon, 'text-primary-600')}></i>
                    {section.title}
                  </h4>
                  <div className="grid md:grid-cols-2 gap-6">
                    {section.fields.map((field, fieldIndex) => (
                      <RangeSlider
                        key={field.key}
                        label={field.label}
                        value={formData[field.key]}
                        onChange={(e) => handleInputChange(field.key, e.target.value)}
                        min={field.min}
                        max={field.max}
                        step={field.step}
                        unit={field.unit}
                        color={field.color}
                        showValue={true}
                      />
                    ))}
                  </div>
                </div>
              ))}

              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-6">
                <Button
                  type="submit"
                  variant="primary"
                  size="lg"
                  loading={isCalculating}
                  disabled={connectionStatus !== 'connected'}
                  icon="fas fa-chart-line"
                  className="w-full sm:w-auto"
                >
                  {isCalculating ? 'Analyse en cours...' : 'Évaluer mon risque'}
                </Button>
                
                <Button
                  type="button"
                  variant="ghost"
                  size="lg"
                  icon="fas fa-redo"
                  onClick={resetForm}
                  className="w-full sm:w-auto"
                >
                  Réinitialiser
                </Button>
              </div>

              {connectionStatus !== 'connected' && (
                <div className="text-center mt-4">
                  <p className="text-sm text-warning-600 flex items-center justify-center">
                    <i className="fas fa-info-circle mr-2"></i>
                    Le serveur ML doit être démarré pour utiliser cette fonctionnalité
                  </p>
                </div>
              )}
            </form>
          </CardBody>
        </Card>

        {/* Results */}
        {showResults && results.score > 0 && (
          <Card className="animate-scale-in">
            <CardHeader className="text-center pb-6">
              <h3 className="text-2xl font-bold text-secondary-900 mb-2">
                <i className="fas fa-chart-pie mr-2 text-primary-600"></i>
                Résultats de l'Analyse
              </h3>
              <p className="text-secondary-600">
                Basé sur notre modèle de machine learning
              </p>
            </CardHeader>
            
            <CardBody className="space-y-6">
              {/* Score Cards */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-primary-50 rounded-xl p-4 text-center hover-lift">
                  <div className="text-3xl font-bold text-primary-600 mb-1">
                    {results.score}
                  </div>
                  <div className="text-sm text-primary-700 font-medium">Score Global</div>
                  <div className="text-xs text-primary-600">/ 100</div>
                </div>
                
                <div className={cn(
                  'rounded-xl p-4 text-center hover-lift',
                  results.prediction === 'Addicted' 
                    ? 'bg-accent-50' 
                    : 'bg-success-50'
                )}>
                  <div className={cn(
                    'text-2xl font-bold mb-1',
                    results.prediction === 'Addicted' 
                      ? 'text-accent-600' 
                      : 'text-success-600'
                  )}>
                    {results.prediction}
                  </div>
                  <div className={cn(
                    'text-xs font-medium',
                    results.prediction === 'Addicted' 
                      ? 'text-accent-700' 
                      : 'text-success-700'
                  )}>
                    {results.confidence}
                  </div>
                </div>
                
                <div className="bg-secondary-50 rounded-xl p-4 text-center hover-lift">
                  <div className="text-2xl font-bold text-secondary-700 mb-1">
                    {results.riskLevel}
                  </div>
                  <div className="text-xs text-secondary-600">Niveau de Risque</div>
                </div>
                
                <div className="bg-info-50 rounded-xl p-4 text-center hover-lift">
                  <div className="text-2xl font-bold text-info-600 mb-1">
                    {results.processingTime > 0 ? `${results.processingTime.toFixed(0)}ms` : 'N/A'}
                  </div>
                  <div className="text-xs text-info-600">Temps de Traitement</div>
                </div>
              </div>

              {/* Feature Importance */}
              <div>
                <h4 className="text-lg font-semibold text-secondary-900 mb-4 flex items-center">
                  <i className="fas fa-chart-bar mr-2 text-primary-600"></i>
                  Analyse des Caractéristiques
                </h4>
                <div className="space-y-2">
                  {Object.entries(results.categoryScores).slice(0, 6).map(([key, value], index) => (
                    <div key={key} className="flex items-center space-x-3">
                      <div className="w-32 text-sm text-secondary-600 truncate">
                        {key.replace(/([A-Z])/g, ' $1').replace(/_/g, ' ').trim()}
                      </div>
                      <div className="flex-1">
                        <div className="progress">
                          <div 
                            className={cn(
                              'progress-bar',
                              ['bg-primary', 'bg-secondary', 'bg-accent', 'bg-success', 'bg-warning', 'bg-info'][index % 6]
                            )}
                            style={{ width: `${Math.abs(value || 0) * 100}%` }}
                          ></div>
                        </div>
                      </div>
                      <div className="w-12 text-sm text-right text-secondary-600">
                        {Math.round((value || 0) * 100)}%
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            </CardBody>
            
            <CardFooter className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button
                variant="secondary"
                icon="fas fa-save"
                onClick={saveToLocalStorage}
                className="w-full sm:w-auto"
              >
                Sauvegarder
              </Button>
              
              <Button
                variant="secondary"
                icon="fas fa-download"
                onClick={downloadReport}
                className="w-full sm:w-auto"
              >
                Télécharger
              </Button>
              
              <Button
                variant="outline"
                icon="fas fa-share-alt"
                onClick={shareResults}
                className="w-full sm:w-auto"
              >
                Partager
              </Button>
            </CardFooter>
          </Card>
        )}
      </main>

      {/* Footer */}
      <footer className="bg-white border-t border-secondary-200 mt-16">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center text-secondary-600">
            <p className="mb-2">
              <i className="fas fa-shield-alt mr-2"></i>
              Vos données sont traitées localement et ne sont jamais partagées
            </p>
            <p className="text-sm">
              Powered by Machine Learning • Accuracy: 97.94%
            </p>
          </div>
        </div>
      </footer>
    </div>
  );
};

export default AddictionFormModern;
